    ...
    <div class="max-w-3xl">
      {#each items as item, i (item.question)}
        <div class="border-brand-border border-t">
          <button
            onclick={() => toggle(i)}
            class="text-brand-ink focus-visible:ring-brand-accent flex w-full items-center justify-between gap-4 py-4 text-left font-sans text-[15px] font-semibold transition-colors duration-150 hover:text-purple-700 focus-visible:ring-2 focus-visible:outline-none"
            aria-expanded={openIndex === i}
          >
            <span>{item.question}</span>
            <span
              class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-500 text-sm font-light text-white transition-transform duration-200"
          ...
          {#if openIndex === i}
            <p class="text-brand-muted pb-5 text-[15px] leading-relaxed">
              {item.answer}
            </p>
          {/if}
        </div>
      {/each}
      <div class="border-brand-border border-t"></div>
    </div>
  </div>
</section>
